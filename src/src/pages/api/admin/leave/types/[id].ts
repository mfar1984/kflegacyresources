...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const leaveType = await query('SELECT * FROM leave_types WHERE id = ?', [id]) as Array<any>;
      if (leaveType.length === 0) {
        return res.status(404).json({ error: 'Leave type not found' });
      }

      return res.status(200).json({ leaveType: leaveType[0] });
    }

    if (req.method === 'PUT') {
      if (!permissions.includes('leave_update')) {
        return res.status(403).json({ error: 'Access denied' });
      }

      const { code, name, description, default_days, color, status } = req.body;
      ...
      await query(
        'UPDATE leave_types SET code = ?, name = ?, description = ?, default_days = ?, color = ?, status = ? WHERE id = ?',
        [code, name, description || null, default_days || null, color, status, id]
      );
    ...
    if (req.method === 'DELETE') {
      if (!permissions.includes('leave_delete')) {
        return res.status(403).json({ error: 'Access denied' });
      }
