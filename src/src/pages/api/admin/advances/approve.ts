...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!hasPermission(userPermissions, 'advances_approve')) {
  ...
  try {
    ...
    if (!id || !action || !['approve', 'reject'].includes(action)) {
      return res.status(400).json({ error: 'Advance ID and valid action (approve/reject) are required' });
    }
    ...
    if (advance.status !== 'pending') {
      return res.status(400).json({ error: 'Only pending advances can be approved/rejected' });
    }
    ...
    return res.status(200).json({ 
      message: `Advance ${action === 'approve' ? 'approved' : 'rejected'} successfully` 
    });
  } catch (error) {
    console.error('Advance approval error:', error);
    return res.status(500).json({ error: 'Failed to process advance approval' });
  }
}
