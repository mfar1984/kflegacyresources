...
export default async function handler(
  ...
  try {
    ...
    const [rows] = await db.query(
      'SELECT * FROM certificates WHERE view_hash = ? AND is_active = 1',
      [hash]
    );
    ...
    // Read original PDF
    const pdfPath = path.join(process.cwd(), 'public', certificate.file_path);
    ...
    // Set PDF metadata
    pdfDoc.setTitle(`ANSAR TECHNOLOGIES SDN BHD - ${certificate.certificate_name} (CONFIDENTIAL)`);
    pdfDoc.setSubject(`${certificate.certificate_name} - Confidential`);
    pdfDoc.setAuthor('ANSAR TECHNOLOGIES SDN BHD');
    pdfDoc.setKeywords(['confidential', 'protected', certificate.category.toLowerCase()]);
    ...
    res.setHeader('Content-Disposition', `inline; filename="${certificate.certificate_name}-confidential.pdf"`);
