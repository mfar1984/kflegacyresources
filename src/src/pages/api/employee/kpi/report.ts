...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    drawText('Code', 40, y, 10, true); drawText('Title', 100, y, 10, true); drawText('Target', 340, y, 10, true); drawText('Actual', 400, y, 10, true); drawText('Weight', 460, y, 10, true); drawText('Score %', 520, y, 10, true); y -= 12;
    const toNum = (v: any) => (v == null ? 0 : typeof v === 'number' ? v : parseFloat(String(v)) || 0);
    assignments.forEach((a: any) => {
      const actual = toNum(a.latest_actual); const target = toNum(a.target_value); let score = 0; if (a.unit === 'boolean') score = actual ? 100 : 0; else score = target > 0 ? Math.min(actual / target, 1) * 100 : 0;
      drawText(String(a.code || ''), 40, y, 10); drawText(String(a.title || ''), 100, y, 10); drawText(String(target), 340, y, 10); drawText(String(actual), 400, y, 10); drawText(String(a.weight), 460, y, 10); drawText(String(score.toFixed(2)), 520, y, 10); y -= 12;
    });
    y -= 12; drawText(`Final Score: ${result.final_score != null ? Number(result.final_score).toFixed(2) : '-'}`, 40, y, 12, true); y -= 16; drawText(`Grade: ${result.grade || '-'}`, 40, y, 12, true); y -= 16; drawText(`KPI Bonus: RM ${Number(result.bonus_amount || 0).toFixed(2)}`, 40, y, 12, true);
    ...
    console.error('Employee KPI report error:', error);
