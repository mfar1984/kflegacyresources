...
export default function ClaimTypesManagement({ sessionHash }: { sessionHash: string }) {
  ...
  const fetchClaimTypes = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/admin/claims/types?hash=${sessionHash}`);
      if (response.ok) {
        const data = await response.json();
        setClaimTypes(data.claimTypes || data.types || []);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const url = showEditModal && selectedType 
      ? `/api/admin/claims/types/${selectedType.id}?hash=${sessionHash}`
      : `/api/admin/claims/types?hash=${sessionHash}`;
    ...
    try {
      ...
      if (response.ok) {
        alert(`Claim type ${showEditModal ? 'updated' : 'created'} successfully`);
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Delete this claim type?')) return;
    try {
      const response = await fetch(`/api/admin/claims/types/${id}?hash=${sessionHash}`, { method: 'DELETE' });
      if (response.ok) {
  ...
  const openEditModal = (type: ClaimType) => {
  ...
  const filteredTypes = claimTypes.filter((type) => {
  ...
  const tableColumns: AdminTableColumn<ClaimType>[] = useMemo(() => [
    {
      key: 'code',
      label: 'Code',
      render: (_, row: ClaimType) => (
        <span style={{ backgroundColor: `${row.color}15`, color: row.color, padding: '4px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 600 }}>
          {row.code}
        </span>
      )
    },
    {
      key: 'name',
      label: 'Claim Type',
      render: (value) => <div style={{ fontSize: '12px', fontWeight: 500, color: '#1f2937' }}>{String(value)}</div>
    },
    {
      key: 'max_amount',
      label: 'Max Amount',
      render: (value) => <div style={{ fontSize: '12px', color: '#4b5563' }}>{value ? `RM ${Number(value).toFixed(2)}` : 'No limit'}</div>
    },
    {
      key: 'requires_receipt',
      label: 'Receipt Required',
      align: 'center',
      render: (value) => value ? <i className="bi bi-check-circle-fill" style={{ color: '#10b981', fontSize: '14px' }}></i> : <i className="bi bi-x-circle-fill" style={{ color: '#6b7280', fontSize: '14px' }}></i>
    },
    {
      key: 'status',
      label: 'Status',
    ...
    },
    {
      ...
      render: (_, row: ClaimType) => (
        <AdminTableActions actions={[
          actionPresets.view(() => { setSelectedType(row); setShowViewModal(true); }),
          ...(canPerformAction('claim_types', 'edit') ? [actionPresets.edit(() => { setSelectedType(row); setFormData({ code: row.code, name: row.name, description: row.description, requires_receipt: row.requires_receipt, max_amount: row.max_amount?.toString() || '', color: row.color, status: row.status }); setShowEditModal(true); })] : []),
          ...(canPerformAction('claim_types', 'delete') ? [actionPresets.delete(() => handleDelete(row.id))] : [])
        ]} />
      )
    }
  ...
  if (loading) {
    return (
      <div className="text-center py-5">
        ...
        <p className="text-muted mb-0" style={{ fontSize: 12 }}>Loading claim types...</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-start mb-3">
        <div>
          <h5 className="mb-1" style={{ fontSize: '18px', fontWeight: 600 }}>Claim Types Management</h5>
          <p className="text-muted mb-0" style={{ fontSize: '13px' }}>Manage types of claims available for employees</p>
        </div>
        <PermissionButton sessionHash={sessionHash} module="claim_types" action="create" className="btn btn-primary"
          onClick={() => setShowCreateModal(true)} style={{ fontSize: '11px', padding: '8px 16px', borderRadius: '6px' }}>
          <i className="bi bi-plus-circle me-2"></i>Add Claim Type
        </PermissionButton>
      </div>
      ...
      <form className="row g-3 mb-4">
        ...
        <div className="col-md-2">
          <button type="button" onClick={() => { setSearchQuery(''); setFilterStatus(''); }} className="btn btn-outline-danger w-100"
            style={{ fontSize: '12px', padding: '8px 12px', borderRadius: '6px', fontWeight: 500 }}>
            Reset
          </button>
        </div>
      </form>
      ...
      <div style={{ marginBottom: '24px' }}>
        <AdminTable<ClaimType> columns={tableColumns} data={paginatedTypes} emptyMessage="No claim types found." />
      </div>
            ...
            <div className="modal-content">
              <div className="modal-header" style={{ borderBottom: '1px solid #e5e7eb', padding: '16px' }}>
                <h5 className="modal-title" style={{ fontSize: '14px', fontWeight: 600 }}>
                  ...
                  {showCreateModal ? 'Add Claim Type' : 'Edit Claim Type'}
                </h5>
                  ...
                  <div className="row g-3">
                    <div className="col-md-4">
                      ...
                      <input type="text" className="form-control" required maxLength={10} value={formData.code}
                        onChange={(e) => setFormData({ ...formData, code: e.target.value.toUpperCase() })} placeholder="e.g., TRV"
                        style={{ fontSize: '11px', padding: '8px 12px', borderRadius: '2px' }} />
                    </div>
                    <div className="col-md-8">
                      <label style={{ fontSize: '11px', fontWeight: 500, color: '#374151', marginBottom: '6px' }}>
                        Claim Type Name <span style={{ color: '#ef4444' }}>*</span>
                      </label>
                      <input type="text" className="form-control" required value={formData.name}
                        onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g., Travel Claim"
                        style={{ fontSize: '11px', padding: '8px 12px', borderRadius: '2px' }} />
                    </div>
                    ...
                    <div className="col-md-4">
                      <label style={{ fontSize: '11px', fontWeight: 500, color: '#374151', marginBottom: '6px' }}>Max Amount (RM)</label>
                      <input type="number" step="0.01" className="form-control" value={formData.max_amount}
                        onChange={(e) => setFormData({ ...formData, max_amount: e.target.value })} placeholder="Leave empty for no limit"
                        style={{ fontSize: '11px', padding: '8px 12px', borderRadius: '2px' }} />
                      <small style={{ fontSize: '10px', color: '#6b7280' }}>Empty = No Limit</small>
                    </div>
                    <div className="col-md-4">
                    ...
                    <div className="col-md-12">
                      <div className="form-check" style={{ padding: '12px', backgroundColor: '#f9fafb', borderRadius: '2px' }}>
                        <input type="checkbox" className="form-check-input" id="requires_receipt" checked={formData.requires_receipt}
                          onChange={(e) => setFormData({ ...formData, requires_receipt: e.target.checked })} />
                        <label className="form-check-label" htmlFor="requires_receipt" style={{ fontSize: '11px', fontWeight: 500 }}>
                          <i className="bi bi-receipt me-2"></i>Receipt Required
                        </label>
                      </div>
                    </div>
                  </div>
        ...
        <div className="modal show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }} tabIndex={-1}>
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header" style={{ borderBottom: '1px solid #e5e7eb', padding: '16px' }}>
                <h5 className="modal-title" style={{ fontSize: '14px', fontWeight: 600 }}>
                  <i className="bi bi-info-circle me-2"></i>Claim Type Details
                </h5>
              ...
              <div className="modal-body" style={{ padding: '24px' }}>
                <div className="row g-3">
                  <div className="col-md-4">
                    ...
                    <div><span style={{ backgroundColor: `${selectedType.color}15`, color: selectedType.color, padding: '6px 12px', borderRadius: '2px', fontSize: '12px', fontWeight: 600 }}>{selectedType.code}</span></div>
                  </div>
                  <div className="col-md-8">
                  ...
                  <div className="col-md-6">
                    <label style={{ fontSize: '10px', color: '#6b7280', marginBottom: '4px' }}>Max Amount</label>
                    <div style={{ fontSize: '12px', fontWeight: 500 }}>
                      {selectedType.max_amount ? `RM ${selectedType.max_amount.toLocaleString('en-MY', { minimumFractionDigits: 2 })}` : 'No Limit'}
                    </div>
                  </div>
                  <div className="col-md-6">
                    <label style={{ fontSize: '10px', color: '#6b7280', marginBottom: '4px' }}>Status</label>
                    <div><span style={{ backgroundColor: selectedType.status === 'active' ? '#10b981' : '#6b7280', color: '#ffffff', padding: '4px 8px', borderRadius: '2px', fontSize: '10px', fontWeight: 500 }}>{selectedType.status === 'active' ? 'Active' : 'Inactive'}</span></div>
                  </div>
                  <div className="col-md-12">
                    <label style={{ fontSize: '10px', color: '#6b7280', marginBottom: '4px' }}>Receipt Required</label>
                    <div style={{ fontSize: '12px' }}>
                      {selectedType.requires_receipt ? <span style={{ color: '#10b981' }}><i className="bi bi-check-circle-fill me-1"></i>Yes</span> : <span style={{ color: '#6b7280' }}><i className="bi bi-x-circle-fill me-1"></i>No</span>}
                    </div>
                  </div>
                </div>
              </div>
