...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!hasPermission(userPermissions, 'commissions_approve')) {
  ...
  try {
    ...
    if (!id || !action || !['approve', 'reject'].includes(action)) {
      return res.status(400).json({ error: 'Commission ID and valid action (approve/reject) are required' });
    }
    ...
    if (commission.status !== 'pending') {
      return res.status(400).json({ error: 'Only pending commissions can be approved/rejected' });
    }
    ...
    return res.status(500).json({ error: 'Failed to process commission approval' });
  }
}
