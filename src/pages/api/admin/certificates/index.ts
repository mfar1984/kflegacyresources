...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    for (const file of uploadedFiles) {
      ...
      await db.query(
        `INSERT INTO certificates (certificate_name, category, file_type, file_size, description, file_path, view_hash, is_active) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [certificate_name, category, fileExt.toUpperCase(), fileSize, description, filePath, viewHash, is_active || 1]
      );
    }

    res.status(201).json({ message: 'Certificate(s) created successfully' });
  } catch (error) {
