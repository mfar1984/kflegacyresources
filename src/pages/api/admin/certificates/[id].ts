...
async function handleGet(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    res.status(200).json({ certificate: (rows as any[])[0] });
  } catch (error) {
...
async function handlePut(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!certificate_name || !category || !description) {
    ...
    if (uploadedFiles.length > 0) {
      ...
      const [oldRows] = await db.query('SELECT file_path FROM certificates WHERE id = ?', [id]);
      ...
      // Process first file for update (only one certificate can be updated at a time)
      const typedFile = uploadedFiles[0] as FormidableFile;

      // Validate PDF only
      const originalName = typedFile.originalFilename || 'certificate';
      ...
      const sanitizedName = certificate_name.replace(/[^a-z0-9]/gi, '_').toLowerCase();
      ...
      const filePath = `/uploads/certificates/${newFileName}`;
      
      await db.query(
        `UPDATE certificates 
         SET certificate_name = ?, category = ?, file_type = ?, file_size = ?, description = ?, file_path = ?, is_active = ?
         WHERE id = ?`,
        [certificate_name, category, fileExt.toUpperCase(), fileSize, description, filePath, is_active || 1, id]
      );
    } else {
      // Update without changing file
      await db.query(
        `UPDATE certificates 
         SET certificate_name = ?, category = ?, description = ?, is_active = ?
         WHERE id = ?`,
        [certificate_name, category, description, is_active || 1, id]
      );
    }
