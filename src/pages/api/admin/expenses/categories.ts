...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const categories = await query('SELECT * FROM expense_categories ORDER BY code ASC') as Array<any>;
      return res.status(200).json({ categories, expenseCategories: categories });
    }

    if (req.method === 'POST') {
      // Check permission
      if (!permissions.includes('expenses_create')) {
        return res.status(403).json({ error: 'Access denied' });
      }
      ...
      await query(
        'INSERT INTO expense_categories (code, name, description, requires_approval, approval_threshold, color, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [code, name, description || null, requires_approval ? 1 : 0, approval_threshold || null, color || '#6366f1', status || 'active']
      );

      return res.status(201).json({ message: 'Expense category created successfully' });
    }
    ...
    if (req.method === 'PUT') {
      ...
      const { id, code, name, description, requires_approval, approval_threshold, color, status } = req.body;
